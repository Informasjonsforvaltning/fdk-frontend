...
<Alert
    size='sm'
>
    **Good to know**: We need to create the description in a structured data format that the data.norge.no system can understand. This format is called RDF (Resource Description Framework) and is also used by other European data portals and several private companies' data catalogs.
    RDF can be written in various ways – in the examples below, we use the RDF format called Turtle – but you can choose which format to use. You can [read more about RDF here](/rdf).
</Alert>
...
All dataset descriptions must have at least one theme indicated with `dcat:theme`, and the theme must point to either the Data Theme or the EuroVoc code list. In addition, you can point to themes from the LOS code list managed by Digdir.
...
We also have a page on [how to use codes](/docs/tutorials/code-lists) to tag datasets thematically. Feel free to check it out if you have any questions.
...
<Alert
    size='sm'
>
    **Tip**: To find out where there might be syntax errors, you can check your description on [https://www.easyrdf.org/converter](https://www.easyrdf.org/converter).
    It provides feedback on which line contains an error, but it doesn't know whether your description complies with DCAT-AP-NO.
</Alert>
