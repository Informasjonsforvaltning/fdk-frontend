...
1. **By internal FDK ID**: Use the endpoints `/v1/{type}/{id}` where `{id}` is the internal ID of the resource. These IDs are available from the [search endpoint](/en/technical/api/search) and [AI search endpoint](/en/technical/api/ai-search).

2. **By RDF URI**: Use the endpoints `/v1/{type}/by-uri?uri={uri}` or `/v1/{type}/by-uri/graph?uri={uri}` where `{uri}` is the RDF URI of the resource. You can find the URI in the resource's RDF graph.
