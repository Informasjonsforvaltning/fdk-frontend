...
<Ingress>
    Ressurstenesta gjev deg direkte tilgang til ressursar frå data.norge.no gjennom REST API. Du kan hente ut datasett, omgrep, datatenester, informasjonsmodellar, tenester og hendingar, både som JSON og RDF-grafer. Tenesta som er ansvarleg for å handtere ressurstenesta er [fdk-resource-service(Github)](https://github.com/Informasjonsforvaltning/fdk-resource-service).
</Ingress>
...
- **Tenester** (`/v1/services/{id}`) - Hent ein spesifikk offentleg teneste frå katalogen
...
1. **Ved intern FDK-ID**: Bruk endepunkta `/v1/{type}/{id}` der `{id}` er den interne ID-en til ressursen. Desse ID-ane er tilgjengelege frå [søkeendepunktet](/nn/technical/api/search) og [KI-søkeendepunktet](/nn/technical/api/ai-search).

2. **Ved RDF URI**: Bruk endepunkta `/v1/{type}/by-uri?uri={uri}` eller `/v1/{type}/by-uri/graph?uri={uri}` der `{uri}` er RDF-URI-en til ressursen. URI-en finn du i RDF-grafen til ressursen.
