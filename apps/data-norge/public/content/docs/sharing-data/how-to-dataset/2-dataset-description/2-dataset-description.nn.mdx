...
<Alert>
  **Nyttig å vite**: Vi må lage beskrivinga i eit strukturert dataformat som Data.norge.no sitt system kan forstå. Dette
  formatet heiter RDF (Resource Description Framework) og er i bruk òg blant andre europeiske dataportalar og i fleire
  private verksemder sine datakatalogar. RDF kan skrivast på fleire ulike måtar – i døma under brukar vi RDF-formatet
  som heiter Turtle – men du vel sjølv kva for eitt du vil bruka. [Du kan lese meir om RDF her](/docs/sharing-data/rdf)
</Alert>
...
Vi vil no leggje til tittelen vi har førebudd, både på bokmål, nynorsk og engelsk. Då brukar vi eigenskapen `dct:title`, og kan angi språket for teksten med `@nb` for bokmål, `@nn` for nynorsk og `@en` for engelsk. Resultatet blir slik:
...
Og tilsvarande vil vi leggje til ei tekstleg beskriving på bokmål, nynorsk og engelsk. Til det brukar vi eigenskapen `dct:description`
...
Vi vil no seie at Digitaliseringsdirektoratet, med organisasjonsnummer 991825827 er utgjevar av datasettet. Vi brukar då eigenskapen `dct:publisher` og peiker til ein URI som representerer Digdir: https://organization-catalogue.fellesdatakatalog.digdir.no/organizations/991825827.
...
<Alert>**Tips**: Du kan leggje inn kommentarar i Turtle med teiknet `#`, då blir det som kjem etter ignorert.</Alert>
...
Datasettet kan knytast til nokre nøkkelord, for eksempel "kunstig intelligens" og "offentleg sektor". Vi brukar eigenskapen `dct:keyword` og lagar tekstar både på bokmål, nynorsk og engelsk.
...
Vi vil seie at datasettet er basert på innsamling frå tredjepartar, og at datasettet har open tilgang. For å beskrive datasettet sitt opphav brukar vi eigenskapen `prov:wasGeneratedBy` og peiker til ein kode som representerer "Innsamla frå tredjepartar". For å uttrykke at datasettet er ope brukar vi eigenskapen `dct:accessRights` og peiker til eksisterande koder som representerer "Open data".
