...
<Alert>
  **Merk**: Vi skiller mellom Datasett, Distribusjon og Datatjeneste/API for å tydeleggjere at eit datasett kan
  tilgjengeleggjast på fleire måtar. I denne samanhengen er datasett ein ganske abstrakt ting, medan dei konkrete filene
  og endepunkta du kan hente dataen frå kallast for distribusjon og datatjeneste.
</Alert>

Vi må fyrst seie at datasettet **har** ei distribusjon, til det brukar vi `dcat:distribution` som skal peike til ein Distribusjonsressurs vi beskriver under.
...
Vi leggjer til ei tekstleg beskrivning av distribusjonen, og når den blei utgitt, og brukar felta `dct:description` og `dct:issued` til det:
...
Vi vil oppgi formatet til fila, som i vårt tilfelle er ei CSV-fil; til det brukar vi eigenskapen `dct:format` og peiker til ein kode i EU sitt vokabular "File Type" som representerer CSV.
