...
export const parseSearchPageSizeParam = function (value: string | null): number {
  ...
  const parsed = Number.parseInt(value, 10);
  if (!Number.isFinite(parsed)) return SEARCH_DEFAULT_PAGE_SIZE;
  return (SEARCH_PAGE_SIZE_OPTIONS as readonly number[]).includes(parsed) ? parsed : SEARCH_DEFAULT_PAGE_SIZE;
};
