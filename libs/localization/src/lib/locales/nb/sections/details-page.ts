const detailsPage = {
  breadcrumbs: {
    datasets: "Datasett",
    services: "Tjenester"
  },
  header: {
    namelessOrganization: "Navnløs virksomhet",
    namelessDataset: "Navnløst datasett",
    namelessService: "Navnløs tjeneste",
    useDatasetButton: "Bruk datasett",
    datasetsTagLink: "Datasett",
    servicesTagLink: "Tjenester",
    published: "Publisert",
    requestAccessButton: "Be om tilgang",
    showInterestButton: "Meld interesse"
  },
  useDatasetPopover: {
    distributions: "Distribusjoner",
    apis: "API-er"
  },
  resourceNotAvailableNotice: {
    title: "Dette datasettet er enda ikke tilgjengelig",
    body: "Dette datasettet har ingen distribusjoner. Det betyr at kun beskrivelsen av datasettet er tilgjengelig, mens selve innholdet og dataene ennå ikke er publisert. Du kan melde din interesse for datasettet til den ansvarlige virksomheten ved å klikke på knappen nedenfor. Jo flere som viser interesse, desto større er sjansen for at dataene blir gjort tilgjengelige.",
    moreInfoLink: "Mer informasjon"
  },
  accessRights: {
    helpTextTitle: "Begrepsforklaring",
    unknownLabel: "Ukjent tilgangsnivå",
    readMoreLinkText: "Les mer om tilgangsnivåer her",
    codes: {
      NON_PUBLIC: {
        label: "Ikke-allmenn tilgang",
        helpText: "Ikke offentlig tilgjengelig av personvern-, sikkerhets- eller andre årsaker. Denne kategorien kan inkludere ressurser som inneholder sensitiv eller personlig informasjon."
      },
      RESTRICTED: {
        label: "Begrenset tilgang",
        helpText: "Kun tilgjengelig under visse betingelser. Denne kategorien kan inkludere ressurser som krever betaling, ressurser delt under taushetsavtaler, eller ressurser der utgiver eller eier ennå ikke har bestemt om de kan offentliggjøres."
      },
      PUBLIC: {
        label: "Allmenn tilgang",
        helpText: "Offentlig tilgjengelig for alle. Tilgang kan likevel kreve registrering og forespørsel om API-nøkler, så lenge hvem som helst kan be om slik registrering og/eller API-nøkler."
      }
    }
  },
  tabs: {
    overview: "Oversikt",
    distributions: "Distribusjoner og API-er",
    details: "Detaljer",
    community: "Diskusjoner",
    rdf: "RDF"
  },
  overview: {
    description: {
      title: "Beskrivelse",
      placeholder: "Dette datasettet har ingen beskrivelse"
    }
  },
  distributions: {
    title: "Distribusjoner",
    placeholder: "Dette datasettet har ingen distribusjoner.",
    header: {
      nameless: "Navnløs distribusjon",
      openLicense: "Åpen lisens",
      hasApi: "API",
      exampleData: "Eksempeldata",
      downloadBtnLabel: "Last ned"
    },
    details: {
      description: "Beskrivelse",
      accessURL: "TilgangsURL",
      downloadURL: "Direkte nedlastning",
      accessService: "API",
      page: "Dokumentasjon",
      license: "Lisens",
      conformsTo: "I samsvar med",
      status: "Status",
      rightsType: "Rettigheter for bruk",
      mobilityDataStandard: "Mobility-standard",
      noData: "Ikke oppgitt"
    },
    downloadModal: {
      header: "Denne distribusjonen er tilgjengelig på følgende URL-er:",
      closeBtn: "Lukk vindu"
    }
  },
  produces: {
    title: "Tjenesteresultat",
    nameless: "Navnløst tjenesteresultat",
    placeholder: "Denne tjenesten har ingen tjenesteresultat."
  },
  apis: {
    title: "API-er som tilgjengeliggjør dette datasettet",
    placeholder: "Ingen registrerte API-er tilgjengeliggjør dette datasettet.",
    header: {
      nameless: "Navnløst API",
      gotoBtn: "Gå til API"
    },
    details: {
      noData: "Ikke oppgitt",
      description: "Beskrivelse",
      endpoint: "Endepunkt",
      url: "URL",
      page: "Dokumentasjon"
    }
  },
  similarDatasets: "Lignende datasett",
  internalRelations: "Relasjoner til andre ressurser",
  externalRelations: "Relasjoner fra andre ressurser",
  details: {
    showEmptyRows: "Vis tomme rader",
    hideEmptyRows: "Skjul tomme rader",
    noData: "Ikke oppgitt",
    general: {
      title: "Om datasettet",
      serviceTitle: "Om tjenesten",
      publisher: "Utgiver",
      firstHarvested: "Publisert",
      firstHarvestedHelpText: "Denne datoen sier når datasettet ble høstet av data.norge.no. Det kan ha vært tilgjengelig tidligere andre steder.",
      firstHarvestedHelpTextTitle: "Begrepsforklaring",
      firstHarvestedHelpTextLink: "Les mer om høsting her",
      modified: "Sist oppdatert",
      page: "Dokumentasjon",
      type: "Datasettype",
      landingPage: "Landingsside",
      homepage: "Hjemmeside",
      metadataQuality: {
        title: "Metadatakvalitet",
        helpText: "Metadatakvalitet er en indikator på hvor godt datasettene er beskrevet ved hjelp avmetadata.",
        helpTextTitle: "Begrepsforklaring",
        helpTextLink: "Les mer om metadatakvalitet her",
        labels: {
          excellent: "Utmerket",
          good: "God",
          sufficient: "Tilstrekkelig",
          poor: "Dårlig"
        }
      },
      copyButton: [
        "Kopier",
        "Kopiert!"
      ]
    },
    contactPoint: {
      title: "Kontaktinformasjon",
      formattedName: "Kontaktpunkt",
      uri: "Nettside",
      email: "E-post",
      telephone: "Telefon"
    },
    content: {
      title: "Om dataene",
      language: "Språk",
      qualifiedAttributions: "Innholdsleverandører",
      provenance: "Opphav",
      accrualPeriodicity: "Oppdateringsfrekvens",
      issued: "Først utgitt",
      issuedHelpText: "Denne datoen sier når dataene i dette datasettet første gang ble utgitt. Det kan ha skjedd før datasettet ble publisert på data.norge.no.",
      modified: "Sist oppdatert",
      accuracyAnnotation: "Nøyaktighet",
      availabilityAnnotation: "Tilgjengelighet",
      completenessAnnotation: "Kompletthet",
      currentnessAnnotation: "Aktualitet",
      relevanceAnnotation: "Relevans",
      spatial: "Geografisk avgrensning",
      temporal: "Tidsmessig avgrensning",
      temporalFrom: "Fra",
      temporalTo: "Til",
      conformsTo: "I samsvar med",
      conformsToHelpText: "Referanse til en implementasjonsregel eller annen spesifikasjon, som ligger til grunn for opprettelsen av datasettet."
    },
    legal: {
      title: "Lovhjemler",
      legalBasisForAccess: "Utleveringshjemmel",
      legalBasisForProcessing: "Behandlingshjemmel",
      legalBasisForRestriction: "Skjermingshjemmel"
    },
    concepts: {
      title: "Begreper brukt i datasett"
    },
    subject: {
      title: "Begreper brukt i tjenesten"
    },
    related: {
      title: "Relasjoner til andre ressurser",
      namelessDataset: "Navnløst datasett",
      namelessOrganization: "Navnløs virksomhet"
    },
    requires: {
      title: "Krever",
      placeholder: "Denne tjenesten krever ingen andre tjenester."
    },
    references: {
      title: "Referanser",
      relatedResources: "Relaterte ressurser",
      relationLabels: {
        "http://purl.org/dc/terms/associativeRelation": "Assosiert med",
        "http://purl.org/dc/terms/closeMatch": "Nær match",
        "http://purl.org/dc/terms/exactMatch": "Eksakt match",
        "http://purl.org/dc/terms/generalizes": "Generalisert av",
        "http://purl.org/dc/terms/specializes": "Spesialisert av",
        "http://purl.org/dc/terms/memberOf": "Medlem av",
        "http://purl.org/dc/terms/replaces": "Erstatter",
        "http://purl.org/dc/terms/isReplacedBy": "Erstattet av",
        "http://purl.org/dc/terms/seeAlso": "Se også",
        "http://purl.org/dc/terms/conformsTo": "Overholder",
        "http://purl.org/dc/terms/servesDataset": "Tjener datasett",
        "http://purl.org/dc/terms/inSeries": "I serie",
        "http://purl.org/dc/terms/subject": "Emne",
        "http://purl.org/dc/terms/hasPart": "Har del av",
        "http://purl.org/dc/terms/isPartOf": "Er del av",
        "http://purl.org/dc/terms/isGroupedBy": "Er gruppert av",
        "http://purl.org/dc/terms/isClassifiedBy": "Er klassifisert av",
        "http://purl.org/dc/terms/isDescribedAt": "Er beskrevet på",
        "http://purl.org/dc/terms/relation": "Relasjon",
        "http://purl.org/dc/terms/hasVersion": "Har versjon",
        "http://purl.org/dc/terms/isVersionOf": "Er versjon av",
        "http://purl.org/dc/terms/references": "Refererer til",
        "http://purl.org/dc/terms/isReferencedBy": "Er referert til",
        "http://purl.org/dc/terms/requires": "Krever",
        "http://purl.org/dc/terms/isRequiredBy": "Kreves av",
        "http://purl.org/dc/terms/source": "Kilde",
        unknown: "Ukjent relasjon"
      }
    },
    themes: "Tema",
    mobilityThemes: "Mobility-tema",
    thematicArea: "Temaområde",
    keywords: "Søkord"
  },
  community: {
    title: "Diskusjoner på Datalandsbyen",
    noData: "Ingen diskusjoner funnet",
    notice: {
      title: "Hva er Datalandsbyen?",
      body: "Datalandsbyen er vårt nettforum hvor du kan etterspørre data, dele erfaringer og spørre om råd som gjelder datadeling og informasjonsforvaltning.",
      gotoLink: "Gå til Datalandsbyen",
      moreInfo: "Mer informasjon"
    },
    topicRow: {
      postedBy: "Postet av",
      votes: "Stemmer",
      posts: "Innlegg",
      views: "Visninger"
    }
  },
  rdf: {
    titleHelpText: "Alle URL-er til ressurser på data.norge.no kan levere RDF-metadata i flere ulike formater, avhengig av hvilken Accept header man sender med.",
    titleHelpTextLink: "Les mer om RDF og hvilke formater vi støtter her",
    titleHelpTextTitle: "Begrepsforklaring",
    inputLabel: "Ressurs URI",
    copyButton: [
      "Kopier",
      "Kopiert!"
    ],
    loading: "Laster"
  },
  datasetPreview: {
    generatingPreview: "Genererer forhåndsvisning...",
    previewNotAvailable: "Forhåndsvisning ikke tilgjengelig",
    previewNotAvailableHelpText: "Forhåndsvisning er kun tilgjengelig for datakilder med CSV og XLS/XLSX format.",
    showPreviewButton: "Vis forhåndsvisning",
    showingMaxRows: "Viser maks 100 rader",
    closeButton: "Lukk vindu",
    downloadButton: "Last ned hele datasettet",
    noTableData: "Ingen tabelldata tilgjengelig"
  },
  statusCodes: {
    Completed: "Godkjent",
    UnderDevelopment: "Under utvikling",
    Withdrawn: "Trukket",
    Deprecated: "Utgått",
    Discontinued: "Avsluttet"
  }
};

export default detailsPage;
