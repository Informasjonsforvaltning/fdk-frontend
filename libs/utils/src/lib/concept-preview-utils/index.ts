...
const frameConcept = async (concept: object, frame: Frame): Promise<jsonld.NodeObject> => {
...
export const getSkosLabel = async (concept: object, lang: Locale['code']) => {
    ...
    const conceptLabel = getPrefLabel(framedConcept)
    return conceptLabel
}

export const getSkosDefinition = async (concept: object, lang: Locale['code']) => {
    const framedConcept = await frameConcept(await jsonld.expand(concept), conceptFrame)
    const conceptDirectDefinition = getDirectDefinition(framedConcept, lang)
    const framedXlDef = await frameConcept(await jsonld.expand(concept), xlNoteFrame)
    const conceptXlDefinition = getXlDefinition(framedXlDef, lang)

    return conceptDirectDefinition || conceptXlDefinition
}
